...
export async function GET(){
  ...
  try{
    ...
    const [macroResponse,marketResponse,energyResponse]=await Promise.all([
      baseUrl?fetch(`${baseUrl}/v1/data/macro`,{headers:{"User-Agent":"AULOS-NEWS-Frontend/1.3"}}).catch(()=>null):Promise.resolve(null),
    ...
    let data:Array<{seriesId:string;label:string;date:string;value:number;display:string}>=[];
    if(macroResponse?.ok){
      const macro=await macroResponse.json() as {data?:Array<{seriesId:string;label:string;date:string;value:number;unit:string}>};
      for(const item of macro.data??[]){
        const meta=seriesMeta[item.seriesId];if(!meta)continue;const number=item.value.toFixed(meta.decimals);
        data.push({seriesId:item.seriesId,label:meta.label,date:item.date,value:item.value,display:meta.unit==="$"?`$${number}`:`${number}${meta.unit}`});
      }
    }else{
      const response=await fetch(url,{headers:{"User-Agent":"AULOS-NEWS/0.3 (research intelligence platform)"}});
      if(!response.ok)throw new Error(`FRED returned ${response.status}`);
      data=latestObservations(await response.text());
    }
    ...
    const providers=[marketResponse?.ok?"Alpaca":null,energyResponse?.ok?"EIA":null,macroResponse?.ok?"FRED via AULOS API":"FRED"].filter(Boolean).join(" + ");
